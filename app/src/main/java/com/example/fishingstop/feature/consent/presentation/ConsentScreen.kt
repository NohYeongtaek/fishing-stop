...
fun ConsentScreen(
    ...
    if (showDeclineDialog) {
        AlertDialog(
            ...
            title = { Text("동의하지 않으시겠어요?", style = AppTheme.type.cardLabel, color = colors.textPrimary) },
            text = {
                Text(
                    "피싱멈춰!는 동의하셔야 이용할 수 있어요.\n동의하지 않으면 앱이 종료됩니다.",
            ...
            confirmButton = {
                TextButton(onClick = onDecline) {
                    Text("종료", color = colors.dangerPrimary, style = AppTheme.type.button)
                }
            },
            dismissButton = {
                TextButton(onClick = { showDeclineDialog = false }) {
                    Text("계속 보기", color = colors.greenPrimary, style = AppTheme.type.button)
                }
            }
        )
    }
