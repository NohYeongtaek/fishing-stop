...
abstract class InspectModule {
    ...
    companion object {
        ...
        private val URL_CONTEXT_SYSTEM_INSTRUCTION = """
            ...
            매우 중요(근거의 사실성 — 지어내지 마라): signals에는 실제로 가져온 페이지 내용에서
            관찰한 것만 담아라. 페이지를 가져오지 못했다면 "가져오지 못함"이라는 사실만 담고,
            내용을 추측해서 서술하지 마라.
            ...
            페이지를 못 가져온 경우의 처리(중요): 대형 쇼핑몰·은행·공공기관 사이트는 봇 차단
            정책으로 자동화 도구의 접근을 흔히 막는다 — 접근 차단 자체는 그 사이트가 위험하다는
            신호가 아니다. 다른 의심 요소(도메인의 브랜드 사칭·오탈자·비정상 TLD 등) 없이 단순히
            페이지를 못 가져온 경우는 riskScore 0~20(안전에 가까움)으로 두고, "직접 브라우저나
            공식 앱으로 확인하라"는 안내만 담아라. 도메인 자체에 별도 의심 요소가 함께 있을 때만
            점수를 올려라.

            판정 기준:
            - 안전(riskScore 0~30, riskLevel "SAFE"): 피싱 징후 없는 정상 서비스 페이지, 또는
              다른 의심 요소 없이 페이지 접근만 차단된 경우.
